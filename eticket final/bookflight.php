...
<li class="nav-item">
 ...
 <?php 
	...
	$db=new DatabaseCon();
	$fid=$_GET['bid'];
	$s="select fname from tbl_flight where fid='$fid'";
	$rs=$db->selectData($s);
$row=mysql_fetch_array($rs);
$fname=$row['fname'];
?>

<form action="bookflaction.php" method="get">


<tr> <th>Flight Name</th> <td> <input type="text" required name="fn" value="<?php echo $fname; ?>" readonly class="email-bt"> </td> </tr>


<tr> <th>source</><td> <input type="text" name="src" required readonly value="
	 <?php
	$s="select fsrc from tbl_flight where fid='$fid'";
	$rs=$db->selectData($s);
	$row=mysql_fetch_array($rs);
	$src=$row['fsrc'];
	echo $src;
...
<tr> <th>destination</th> <td><input type="text" name="dest" class="email-bt" required readonly value="
	 <?php
	$s="select fdest from tbl_flight where fid='$fid'";
	$rs=$db->selectData($s);
	$row=mysql_fetch_array($rs);
	$dest=$row['fdest'];
	echo $dest;
...
<tr> <th>Journey Date</th> <td><select name="jdate" required class="email-bt">
								...
								$bregno=$_GET['bid'];
								$d=Date('Y-m-d');
								$s="select * from tbl_fshedule where fname='$fname' and fjdate>'$d'";
								$rs=$db->selectData($s);
	while($row=mysql_fetch_array($rs)){
		?>
		<option value="<?php echo $row['fjdate'];?>"><?php echo $row['fjdate'];?></option>
